...
pub(crate) fn register(linker: &mut Linker<PluginStoreState>) -> Result<(), String> {
    linker
        .func_wrap(
            ...
            |mut caller: Caller<'_, PluginStoreState>, ptr: i32, capacity: i32| -> i32 {
                ...
                let Ok(text) = serde_json::to_string(&serde_json::json!({
                    "tags": caller.data().active_interaction.tags,
                })) else {
                    return -3;
                };
        ...
        .map_err(|err| format!("Could not define host_read_context_json: {err}"))?;
